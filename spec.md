...
*   **データベースコントロール（上部ボタン群）**:
    ...
    *   **エクスポート (data.js)**: 現在のデータベース（カスタム編集反映後）を、`data.js` 形式で端末に保存。
...
*   **武器詳細情報モーダル**:
    ...
    *   下部アクションフッターに「削除する」「編集する」「シナジー検索」の3つのコントロールを配置。
...
*   ユーザーが武器の追加、編集、削除を行った場合、カスタマイズされた全データリストがJSON文字列として `localStorage` に保存され、リロード時も保持されます。
