...
*   **データベースコントロール（上部ボタン群）**:
    ...
    *   **Googleスプレッドシート連携**: スプレッドシートURL（固定）からリアルタイム同期を行うモーダルを起動。
    *   **初期設定に戻す**: `localStorage` をクリアし、初期データにリセット。
*   **データベーステーブル**:
    *   **表示列**: 「武器名 / 入手エリア」「武器種」「モチーフキャラ」「操作（詳細）」の4列構成とし、武器名の下に入手エリアタグバッジを一覧表示。
...
*   **Googleスプレッドシート連携モーダル**:
    *   固定のオンラインスプレッドシート（`Endfield 武器マスターデータ`）から最新の武器データをダイレクトに読み込む連携モーダル。
    *   **固定スプレッドシートURL**: 連携先URL（`https://docs.google.com/spreadsheets/d/1dZGyJHG_oK9u5ocKmpHjThh76qvFk0knys9LYsR5Koo/edit?gid=332980878#gid=332980878`）をシステム固定とし、ユーザーによる入力の手間を全廃。
    *   **マルチタブ自動結合エンジン（エリア別効果マスターシート）**: スプレッドシート内の別タブ（「エリア別効果シート」等）を自動検出し、エリアごとに定義された「基礎効果」「付加効果」「スキル効果」のドロッププールを抽出。各武器に設定された各種効果と自動照合・クロスリファレンスし、入手エリアを全自動で判定・アサイン反映。
    *   **ブラウザ直リンク機能**: モーダル内に「🔗 シートを開く」ボタンを配置し、タップで対象スプレッドシートを外部ブラウザで直接閲覧・編集可能。
    *   **アプリ起動時自動同期**: チェックボックスをオンにすると、アプリ起動時に固定スプレッドシートからバックグラウンドで最新データを自動取得して反映。
...
### v1.0.3
*   **「エリア別効果マスターシート」マルチタブ連携（効果別入手エリア自動参照エンジン）**:
    *   スプレッドシート内の別タブ（「エリア別効果」シート）を自動検出し、エリアごとに定義された基礎効果・付加効果・スキル効果のドロッププールを読み込んで、武器マスターデータに設定された各効果から入手エリアを全自動で参照・紐付け結合するエンジンを搭載。
*   **GoogleスプレッドシートURLの固定化**:
    *   指定のオンラインスプレッドシート（`https://docs.google.com/spreadsheets/d/1dZGyJHG_oK9u5ocKmpHjThh76qvFk0knys9LYsR5Koo/edit?gid=332980878#gid=332980878`）をシステム固定URLとして組み込み、入力の手間を排除。「🔗 シートを開く」ボタンからスプレッドシートへの直接アクセスに対応。
