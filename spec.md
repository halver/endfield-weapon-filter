...
*   **Googleスプレッドシート連携モーダル**:
    ...
    *   **マルチタブ自動結合エンジン（エリア別排出表）**: スプレッドシート内の別タブ（「エリア別排出表」等）を自動検出し、武器マスターデータとエリア排出マッピング（`エリア名 ↔ 排出武器`）をアプリ側で全自動クロスリファレンス・統合反映。
...
### v1.0.3
*   **「エリア別排出表」マルチタブ連携（2シート自動結合エンジン）**:
    *   スプレッドシート内の別タブ（「エリア別排出」シート）を自動検出し、`エリア名` と `排出武器` のマッピング表を読み込んで武器マスターデータへ全自動結合・反映するエンジンを搭載。
